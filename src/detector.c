...
{ 
    static char name[20]={""};
    char *q = strrchr(p,'/') + 1;
    strncpy(name,q,26);//注意后面的6，如果你的测试集的图片的名字字符（不包括后缀）是其他长度，请改为你需要的长度（官方的默认的长度是6）
   // printf("%s", name);
    return name;
}
...
void validate_detector_recall(char *datacfg, char *cfgfile, char *weightfile)
{
    ...
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/voc/2018_train.txt");
    list *plist = get_paths(valid_images);
    char **paths = (char **)list_to_array(plist);
    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels)
{
    ...
    while (1) {
        ...
        else
        {
            ...
            if (access("C:\\dl-project\\darknet\\build\\darknet\\x64\\data\\out", 0) == -1)
            {
                if (mkdir("C:\\dl-project\\darknet\\build\\darknet\\x64\\data\\out", 0777))
                {
            ...
            for (int i = 0; i < m; ++i)
            {
                ...
                char b[2048];
                sprintf(b, "C:\\dl-project\\darknet\\build\\darknet\\x64\\data\\out\\%s", GetFilename(path));
                save_image(im, b);
    ...
    //while (1) {
        if (filename) {
            ...
            for (i = 0; i < m; ++i) {
                ...
                int letterbox = 0;
                image sized = resize_image(im, net.w, net.h);
                layer l = net.layers[net.n - 1];//zheli
                float *X = sized.data;
                ...
                draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output);
                char b[2048];
                sprintf(b, "C:\\dl-project\\darknet\\build\\darknet\\x64\\data\\outcamera\\%d", i);
                save_image(im, b);
                printf("save %s successfully!\n", b);
                if (save_labels)
                {
    ...
    while (1) {
        if (filename) {
            ...
            for (int i = 0; i < m; ++i)
            {
                ...
                static int k = 0;
                //sprintf(sre, "predictions_%d", k++);
                sprintf(sre, "C:\\dl-project\\darknet\\build\\darknet\\x64\\data\\out\\%s", GetFilename(path));
                save_image(im, sre);
                if (!dont_show) {
                    show_image(im, "predictions");
                }
...
void run_detector(int argc, char **argv)
{
    ...
    float thresh = find_float_arg(argc, argv, "-thresh",0.05);    // 0.24 0.01
    float hier_thresh = find_float_arg(argc, argv, "-hier", .5); //.5 
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, outfile, ext_output, dont_show);
